...
export default function ThemeSwitcher() {
  ...
  return (
    ...
    >
      ...
      {/* Icons */}
      <span className={`absolute left-1 text-xs transition-opacity duration-200 ${isDark ? 'opacity-0' : 'opacity-100'}`}>
        ☀️
      </span>
      <span className={`absolute right-1 text-xs transition-opacity duration-200 ${isDark ? 'opacity-100' : 'opacity-0'}`}>
        🌙
      </span>
    </button>
  );
